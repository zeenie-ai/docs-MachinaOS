...
<AccordionGroup>
  <Accordion title="Where do I add my API keys?">
    Click the **key icon** in the toolbar to open the Credentials panel. OpenCompany supports 12 LLM providers plus service credentials, including:
    - OpenAI
    - Anthropic (Claude)
    - Google AI (Gemini)
    - OpenRouter, xAI, Groq, Cerebras, DeepSeek, Kimi (Moonshot), Mistral
...
<AccordionGroup>
  ...
  <Accordion title="High memory usage">
    - Restart the services: `company stop` then `company start`
  ...
  <Accordion title="Database locked error">
    ...
    - Restart the backend (`company stop` then `company start`)
