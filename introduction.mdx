...
OpenCompany is an open-source workflow automation platform inspired by n8n, built with React Flow and Python FastAPI. Create powerful automations using a visual node-based editor with real-time execution feedback.
...
<CardGroup cols={2}>
  ...
  <Card title="AI-Powered" icon="brain">
    12 LLM providers with conversation memory, tool calling, and thinking modes
  </Card>
  <Card title="Self-Hosted" icon="server">
    Full control with the `company` CLI, Temporal distributed execution, and encrypted credentials
  </Card>
</CardGroup>
...
### AI Integration
Connect to 12 LLM providers with built-in conversation memory, tool calling, and thinking modes.
...
- **xAI** - Grok models via the OpenAI-compatible native chat path
...
<CardGroup cols={2}>
  <Card title="Installation" icon="download" href="/installation">
    Get OpenCompany running locally in minutes
  </Card>
...
OpenCompany follows n8n-inspired architectural patterns with modular backend services:
...
| Deployment | `company deploy up --provider gcp` (Terraform + `company serve` under systemd) |
...
<Info>
OpenCompany is designed for self-hosting. Your data stays on your infrastructure with automatic cache fallback for any environment.
</Info>
